...
public class StartAllServices {
	...
	String content  =  "{\"RequestInfo\":{\"context\":\"Start Service\"},\"Body\":{\"ServiceInfo\":{\"state\":\"STARTED\"}}}";
	
	@Test
	@Parameters({"ambarihost", "username", "password"})
	public void startAllServices(String ambarihost, String username, String password) throws JSONException, InterruptedException {
		...
		String content = "{\"RequestInfo\":{\"context\":\"Start all services\",\"operation_level\":{\"level\":\"CLUSTER\",\"cluster_name\":\""+clustername+"\"}},\"Body\":{\"ServiceInfo\":{\"state\":\"STARTED\"}}}";
		...
		String startStatus = response.getStatusLine();
		String nl = System.lineSeparator();
		
		System.out.println(body + nl);
		
		System.out.println(startStatus + nl);
		
		if(startStatus.contains("202")){
			System.out.println("Start all services request on "+clustername+" accepted!");
		} else {
			System.out.println("ERROR: Request to start all services on "+clustername+" failed!");
		}
		
		Assert.assertEquals("HTTP/1.1 202 Accepted", startStatus);
