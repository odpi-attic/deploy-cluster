...
# Method to clean specific report file
juLogCleanSomeFile() {
  echo "+++ Removing old junit reports of $1 from: $juDIR "
  rm -f "$juDIR"/$1
}
...
juLog() {
  ...
  ## testcase tag
  content="$content
    <testcase assertions=\"1\" classname=\"com.ibm.biginsights.$suite.$name\" name=\"$name\" time=\"$time\">
    $failure
    <system-out>
<![CDATA[
$out
  ...
  <testsuite failures="0" assertions="$assertions" name="com.ibm.biginsights.$suite" tests="1" errors="$errors" time="$total">
    $content
  </testsuite>
EOF
